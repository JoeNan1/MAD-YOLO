...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.base_conv = nn.ModuleList(BaseConv(in_channels=x, out_channels=256, ksize=1, stride=1) for x in ch)  # stem
    ...
    def initialize_biases(self, prior_prob):
        for conv in self.m_labels:
            b = conv.bias.view(self.na, -1)
            b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
            conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)

        for conv in self.m_conf:
            b = conv.bias.view(self.na, -1)
            b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
            conv.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, ASPP,C3NEXT7,C3NEXT11,C3_cot,C3VOV,BeVOV,ECABottleneck,
                C3COT,COTBottleneck]:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost,C3NEXT7,C3NEXT11,C3_cot,C3VOV,C3COT]:
