...
class Block7(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        ...
        self.stochastic_depth = StochasticDepth(0.1, "row") #0.1 je za tiny
    ...
    def forward(self, x):
        ...
        x = self.act(x)
        x = self.pwconv2(x)
        x = x.permute(0, 3, 1, 2) #bilo je ispod ifa
...
class C3NEXT7(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv_L(c1, c_, 1, 1)
        self.cv2 = Conv_L(c1, c_, 1, 1)
        self.cv3 = Conv_L(2 * c_, c2, 1)
        self.m = nn.Sequential(*[Block7(c_) for _ in range(n)])
...
class Block11(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=11, padding=5, groups=dim)  # depthwise conv
        self.norm = norm_layer(dim)
        self.pwconv1 = nn.Linear(dim, 4 * dim)  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        self.pwconv2 = nn.Linear(4 * dim, dim)
...
class C3NEXT11(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv_L(c1, c_, 1, 1)
        self.cv3 = Conv_L(2 * c_, c2, 1)
        self.m = nn.Sequential(*[Block11(c_) for _ in range(n)])
...
class Bottleneck_cot(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.cot = CoTAttention(dim=c_,kernel_size=3)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cot(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class C3_cot(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cot = nn.Sequential(*(Bottleneck_cot(c_, c_, shortcut, g, e=1.0) for _ in range(1)))
        

    def forward(self, x):
        return self.cv3(torch.cat((self.cot(self.m(self.cv1(x))), self.cv2(x)), 1))
...
class C3VOV(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        if n == 3:
            self.m1 = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0)])
            self.m2 = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0)])
            self.m3 = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0)])
...
class BeVOV(nn.Module):
    def __init__(self, c1, c2, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 2, g)  # act=FReLU(c2)
        # print(c_)
...
class Concat256(nn.Module):
    ...
    def __init__(self, dimension=1):
        ...
        inter_channels = int(self.channels // 4)
    ...
    def forward(self, x):
        ...
        xo = torch.cat((2 * x[1] * wei,2 * x[1] * (1 - wei)),1)

        return xo


class Concat512(nn.Module):
    # Concatenate a list of tensors along dimension
    def __init__(self, dimension=1):
        super().__init__()
        self.d = dimension
