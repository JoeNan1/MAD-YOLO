...
def bboxes_giou(bboxes_a, bboxes_b, xyxy=False):
    ...
    giou = iou - (area_c - (area_a[:, None] + area_b - area_i)) / area_c.clamp(1e-16)

    return giou
    
def bboxes_diou(bboxes_a, bboxes_b, xyxy=False):
    if bboxes_a.shape[1] != 4 or bboxes_b.shape[1] != 4:
        raise IndexError
    ...
    center_dis = (torch.pow(bboxes_a[:,None, 0] - bboxes_b[:, 0], 2) +
                  torch.pow(bboxes_a[:,None, 1] - bboxes_b[:, 1],2))  # center diagonal squared
